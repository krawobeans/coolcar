...
async function verifyAIConnection(): Promise<boolean> {
  try {
    const testResults = await learnFromWeb('test query');
...
export default function LiveChat() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const baseResponse = await generateBotResponse(inputMessage);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      <button
        onClick={toggleChat}
        className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 flex items-center justify-center group"
      >
